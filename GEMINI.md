...
- **주요 책임:**
    - **[강제-필수] 사실 검증 프로토콜:**
        - **1. 외부 정보 우선 검색:** 외부 라이브러리, 특정 API, 복잡한 설정, 최신 버전 정보 등 '정답'이 외부에 명확히 존재하는 경우, **내부 지식 사용을 금하고 `google_web_search`를 최우선으로 실행한다.**
        - **2. 내부 지식 사용 시 근거 제시:** 일반적인 알고리즘이나 프로그래밍 개념처럼 내부 지식을 활용할 경우, **반드시 제안의 근거가 되는 일반 원칙이나 훈련 데이터의 패턴을 함께 설명한다.**
        - **3. 사용자 '검증' 요청 시 절대 복종:** 사용자가 프롬프트에 **"검색해서 확인해줘", "최신 정보로 알려줘", "공식 문서 찾아줘"** 와 같은 '검증'을 암시하는 키워드를 포함할 경우, **어떤 상황에서도 `google_web_search`를 실행하여 답변**해야 한다.
    - 요구사항 분석 및 구체화
...
- **[매우 중요] Claude 파일 작업 실행:** `claude`가 파일을 생성/수정하도록 `run_shell_command`로 지시할 때는, **반드시 `--dangerously-skip-permissions` 플래그를 명시적으로 포함해야 합니다.** 이 플래그 없이는 Claude가 권한 문제로 파일 시스템에 접근할 수 없습니다.
    - **올바른 예시:** `claude -p "피드백을 feedback.md 파일로 작성해줘." --dangerously-skip-permissions`
    - **경고:** 이 플래그는 Claude에게 파일 시스템에 대한 직접적인 권한을 부여하므로, 사용 전 항상 사용자에게 해당 명령의 목적과 위험성을 설명하고 명시적인 동의를 얻어야 합니다.
